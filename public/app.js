        ...
        <div>
          <div class="zone-card-header">
            <div class="zone-domain">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 18px; height: 18px; color: var(--primary);">
...
function renderAccountsTable() {
  ...
  if (accountsList.length === 0) {
    container.innerHTML = `<div style="padding: 32px; text-align: center; color: var(--muted-foreground);">No accounts stored yet. Add one in the tab above.</div>`;
    return;
  }
    ...
    <table class="data-table">
      ...
      <tbody>
        ${accountsList.map(acc => `
          <tr>
            <td><strong style="color: var(--foreground);">${acc.account_name}</strong></td>
            <td>${new Date(acc.created_at).toLocaleDateString()}</td>
            <td>
              <span class="status-badge active" style="font-size: 0.725rem;">
...
async function loadDnsRecords() {
  ...
  container.innerHTML = `<div style="padding: 32px; text-align: center; color: var(--muted-foreground);">Loading DNS records from edge...</div>`;
...
function renderTunnelsGrid() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; color: var(--muted-foreground); padding: 24px;">No tunnels found.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = filtered.map(t => {
    ...
    return `
      <tr>
        <td><strong style="color: var(--foreground);">${t.name}</strong></td>
